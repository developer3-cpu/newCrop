  ...
  return (
    <aside className="fruit-nav top-nav" aria-label={t("categories.grains")} ref={ref as any}>
      <div className="fruit-list">
        {grains.map((g) => {
          const href = hrefMode === "report" ? `/report?grain=${g.key}` : `#intro`;
          return hrefMode === "report" ? (
            <Link key={g.key} className="fruit-link" href={href} prefetch={false} data-clean-nav="true" onClick={handleClick(g.key)}>
            ...
            <a key={g.key} className="fruit-link" href={href} data-clean-nav="true" onClick={handleClick(g.key)}>
